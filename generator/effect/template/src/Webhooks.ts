...
export const verifySignature = (
  ...
  Effect.gen(function* () {
    ...
    const signedContent = new TextEncoder().encode(`${webhookId}.${Math.floor(timestamp)}.${bodyText}`);
    const matched = yield* Effect.tryPromise({
      try: () => matchesSignature(signedContent, webhookSignature, secretValue),
      catch: (cause) => new WebhookVerificationError({ message: "Failed to verify signature", cause }),
    });
    if (!matched) {
    ...
    Effect.flatMap(verifySignature(body, headers, secret), (text) =>
      Effect.flatMap(
        Effect.try({
          ...
          catch: (cause) => new WebhookVerificationError({ message: "Failed to parse webhook payload", cause }),
        }),
...
const matchesSignature = async (
  ...
  const signingKeys = await Promise.all(
    hmacKeys(secret).map((key) =>
      globalThis.crypto.subtle.importKey("raw", key, { name: "HMAC", hash: "SHA-256" }, false, ["verify"]),
    ),
  );
  for (const versionedSignature of header.split(" ")) {
    ...
    for (const signingKey of signingKeys) {
      if (await globalThis.crypto.subtle.verify("HMAC", signingKey, decodedSignature, signedContent)) {
        return true;
      }
    }
  }
